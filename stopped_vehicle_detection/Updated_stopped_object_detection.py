...
bg_image = cv2.imread("742.jpg") # if img is not on same dir than need to add path of that img dir  
...
def get_opencv_result(video_to_process):
    ...
    while True:
      ...
      #  print(type(contours))
        ...
        no_con = len(contours)
        print(no_con," ",bg_count)
        if no_con == 0:
            bg_count += 1

        if bg_count > 1000:
            print("updating bg_image")
        ...
        for contour in contours:
            ...
            if top < y < bottom:
                ...
                cv2.putText(frame, "Dropped Object", (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
